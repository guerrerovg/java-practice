...
public class HashMap<K, V> {
    ...
    private void copy(List<Pair<K, V>>[] newArray, int fromId) {
        if (this.values[fromId] == null) {
            return;
        }

        for (int i = 0; i < this.values[fromId].size(); i++) {
            Pair<K, V> value = this.values[fromId].get(i);
